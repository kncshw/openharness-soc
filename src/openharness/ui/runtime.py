...
async def build_runtime(
    *,
    ...
    # If SOC_ALLOWED_TOOLS is set, strip the registry down to only those tools.
    # This keeps irrelevant tools (bash, agent, cron, etc.) out of the API
    # calls, saving tokens and reducing model confusion on small models.
    import os as _os
    _soc_allowed = _os.environ.get("SOC_ALLOWED_TOOLS", "").strip()
    if _soc_allowed:
        tool_registry.filter_to(set(_soc_allowed.split(",")))
    ...
    if parsed is not None:
        ...
        if result.continue_pending:
            ...
            save_session_snapshot(
                ...
                tools=bundle.tool_registry.to_api_schema(),
            )
