...
def build_runtime_system_prompt(
    ...
    *,
    ...
    When the user supplies a custom system prompt via --system-prompt, we treat
    it as an explicit replacement and SKIP the auto-injected sections (skills
    catalogue, CLAUDE.md, memory, reasoning settings, etc.). The custom prompt
    is the only thing we need — anything else just adds noise that competes for
    a smaller model's attention. Users who want both the defaults AND their own
    additions should use --append-system-prompt instead.
    """
