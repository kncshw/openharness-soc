...
class OpenAICompatibleClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        ...
        # Per-request temperature override via env var. If unset, vLLM uses
        # its server-side default (typically 0.7). For SOC batch tasks where
        # consistency matters more than creativity, 0.1–0.3 is recommended.
        import os as _os
        _temp = _os.environ.get("OH_TEMPERATURE", "").strip()
        if _temp:
            try:
                params["temperature"] = float(_temp)
            except ValueError:
                pass  # ignore bad values, fall through to server default
        if openai_tools:
