...
class ToolRegistry:
    ...
    def filter_to(self, names: set[str]) -> None:
        """Remove all tools whose name is NOT in *names*.

        Used by SOC batch mode to strip irrelevant tools (bash, agent, cron,
        etc.) from the registry before API calls. Fewer tools = fewer tokens
        per turn = less model confusion on small models like Gemma4.
        """
